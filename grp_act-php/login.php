...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($_POST['user_id'])) {
        printErr(1022543, 'You must enter a login ID.', '/act/login'.(!empty($_POST['location']) ? '?location='.htmlspecialchars(urlencode($_POST['location'])) : '')); 
        exit(); 
    }        
    if(empty($_POST['password'])) {
        printErr(1022616, 'You must enter a password.', '/act/login'.(!empty($_POST['location']) ? '?location='.htmlspecialchars(urlencode($_POST['location'])) : '')); 
        exit(); 
    } 
    ...
    if(!is_array($check_login)) {
        if($check_login == 'none' || $check_login == 'fail') {
            printErr(1022611, 'Invalid account ID and password combination.', '/act/login'.(!empty($_POST['location']) ? '?location='.htmlspecialchars(urlencode($_POST['location'])) : '')); 
            exit(); 
        } elseif($check_login == 'ban') {
            printErr(1022812, "Account has been banned.\n\nPlease contact the admin if you need any help.", '/act/login'.(!empty($_POST['location']) ? '?location='.htmlspecialchars(urlencode($_POST['location'])) : '')); 
            exit(); 
        }
        exit();
    } else {
        // Check maintenance mode and official_user status
        if($maintenance_mode && (!isset($check_login['official_user']) || $check_login['official_user'] < 1)) {
            printErr(1022999, 'Site is under maintenance. Only official users can login at this time.', '/act/login'); 
            exit();
        }
        ...
        // Explicitly set official_user status in session
        $_SESSION['official_user'] = $check_login['official_user'];
...
if($maintenance_mode) {
    print '<div class="alert alert-warning">
        <p>The site is currently undergoing maintenance.</p>
        <p>Only official users can access the site at this time.</p>
    </div>';
}

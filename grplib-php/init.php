...
function checkMaintenanceMode() {
    ...
    // Always allowed paths during maintenance
    $allowed_paths = [
        '/act/login',
    ...
    // Check if user is logged in as official user
    $is_official_user = (isset($_SESSION['official_user']) && $_SESSION['official_user'] >= 1);
    
    // If path is allowed, let them through
    if(in_array($current_path, $allowed_paths)) {
        return;
    }
    
    // If not official user and not on allowed path, redirect to maintenance
    if(!$is_official_user) {
        header('Location: /error/maintenance.php', true, 302);
        exit();
    }
}

// Make sure session is started before any checks
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

...
function displayreply($ogpost, $reply) {
    if($reply['hidden_resp'] != '1') {
        ...
        print '<li id="reply-'.$reply['id'].'" data-href'.(!empty($_SESSION['pid']) && $_SESSION['pid'] == $reply['pid']? null :($reply['is_spoiler'] == 1 ? '-hidden' : null)).'="/replies/'.$reply['id'].'" class="'.($reply['pid'] == $ogpost['pid'] ? 'my' : 'other').($reply['is_spoiler'] == 1 ? ($show_spoiler ? '' : ' hidden') : '').($mii['official'] == true ? ' official-user' : null).' trigger" data-journal="'.($reply['is_journal'] == 1 ? '1' : '0').'">
        ...
        if($reply['is_hidden'] == '1' && $reply['hidden_resp'] == '0') {
            ...
            print '<div class="header">
                ...
                <p class="timestamp-container">
                    ...
                    <a class="timestamp" href="/replies/'.$reply['id'].'">'.humanTiming(strtotime($reply['created_at'])).'</a>
                </p>
            </div>
            <p class="reply-content-text">'.htmlspecialchars($reply['body']).'</p>';

            if(!empty($reply['screenshot'])) {
                print '<div class="screenshot-container still-image"><img src="'.htmlspecialchars($reply['screenshot']).'"></div>';
            }
            ...
            if(!empty($_SESSION['pid'])) {
                $myempathy = ($mysql->query('SELECT * FROM empathies WHERE empathies.id = "'.$reply['id'].'" AND empathies.pid = "'.$_SESSION['pid'].'"')->num_rows == 1 ? true : false);
...
function editTemplate($type, $post) {
    ...
    print '<option'.($post['is_spoiler'] == 1 ? ' disabled' : '').' value="spoiler" data-action="/'.$type.'/'.$post['id'].'.set_spoiler">Set as Spoiler</option>
...
?>
<style>
/* Journal styling for posts and replies */
#post-content[data-journal="1"],
li[data-journal="1"] {
    padding-left: 50px;
    position: relative;
}

#post-content[data-journal="1"]:before,
li[data-journal="1"]:before {
    content: "";
    position: absolute;
    left: 20px;
    top: 50%;
